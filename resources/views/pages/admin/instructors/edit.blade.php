...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Edit Instructor</h1>
            <p class="text-gray-600 mt-1">Edit data pengajar dan instruktur pelatihan</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.instructors.show', $instructor) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
    ...
    <div class="bg-white rounded-lg border border-gray-200" x-data="instructorForm()">
        <div class="p-6 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Edit Informasi Instructor</h3>
            <p class="text-sm text-gray-600 mt-1">Update data instructor dengan benar</p>
        </div>

        <form action="{{ route('admin.instructors.update', $instructor) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="internal" {{ old('instructor_type', $instructor->instructor_type) === 'internal' ? 'selected' : '' }}>Internal</option>
                        <option value="vendor" {{ old('instructor_type', $instructor->instructor_type) === 'vendor' ? 'selected' : '' }}>Vendor</option>
                    </select>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('instructorForm', () => ({
            certifications: @json($instructor->certifications->map(fn($cert) => ['name' => $cert->certification_name])),
            ...
            init() {
                // Add one empty certification field if no certifications exist

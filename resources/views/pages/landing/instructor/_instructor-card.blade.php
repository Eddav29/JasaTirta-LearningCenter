...
<div class="bg-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300">
    ...
    <div class="p-6">
        <div class="flex items-start space-x-4">
            {{-- Avatar --}}
            <div class="w-20 h-20 rounded-full flex items-center justify-center text-white font-bold text-xl shrink-0 overflow-hidden">
                @if($instructor->image)
                    <img src="{{ $instructor->image }}" alt="{{ $instructor->name }}" class="w-full h-full object-cover">
                @else
                    <div class="w-full h-full bg-linear-to-br from-blue-500 to-blue-700 flex items-center justify-center">
                        @php
            ...
            <div class="flex-1">
                ...
                @if($isVendor && $instructor->company)
    ...
    <div class="px-6 pb-6 space-y-4">
        ...
        <p class="text-sm text-gray-600 leading-relaxed">
            {{ $instructor->bio ?? 'Pengajar berpengalaman di bidang ' . $instructor->specialization }}
        </p>
        
        {{-- Certifications --}}
        @if($instructor->certifications && $instructor->certifications->count() > 0)
        <div>
            <h4 class="font-medium text-sm mb-2 flex items-center text-gray-900">
                ...
                Sertifikasi ({{ $instructor->certifications->count() }})
            </h4>
